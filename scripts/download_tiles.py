...
def _get_elevation(lon, lat, key):
    res = requests.get(
        ...
        params={"locations": f"{lat},{lon}", "key": key},
    )
    if not res.ok:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-k", "--api-key", help="your Google Maps 3d Tiles API key", required=True
    )
    parser.add_argument(
        "-c",
        "--coords",
    ...
    parser.add_argument(
        "-o", "--out", help="output directory to place tiles in", required=True
    )
    ...
    tiles = list(
        tqdm(
            api.get(
                Sphere(cartesian_from_degrees(*args.coords, elevation), args.radius)
            )
        )
    )
    ...
    for i, t in tqdm(enumerate(tiles), total=len(tiles)):
        with open(outdir / Path(f"{i}.glb"), "wb") as f:
            f.write(t.data)
